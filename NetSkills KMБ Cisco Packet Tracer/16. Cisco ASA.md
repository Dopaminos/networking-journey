...
ASA имеет в себе некоторые преднастроенные элементы вроде DHCP-сервера и имён VLAN-ов, т.к. они для неё обязательны. 
Без имён, ASA не будет работать с VLAN-ами.

Настроим лог:пасс командами `ena p cisco` и `us admin p cisco` и проверим `sh r`. 
![](./Картинки/ASA3.png)
...
Этот показатель частично связан с функцией Stateful Inspection, ограничивающий доставку пакетов в сети. 
